...
class Usuario:
	...
	def registrar(self): # Lógica para realizar el registro
		...
		dictnombre[self.id] = self.username
...
def menu_select_articulos(usuario):
	...
	if(articuloID.isnumeric() and dictarticulos.get(articuloID) != None): # Si la entrada es valida y el artículo existe
		menu_articulo(articuloID, usuario)	# Abrir el artículo específico.
	else:
...
def registrar_usuario():
	...
	id = 1+len(dictusuarios)	# Calcular ID (No se pueden borrar usuarios)
	nombre = input("Nombre: ")
...
def mostrar_comentarios(articulo_id):
    ...
    if comentarios_articulo:
        ...
        for comentario in comentarios_articulo:
            print(f" autor_id:{comentario.id}\n comentario: {comentario.contenido}\n"
		  f" autor:{comentario.id_usuario}\n Fecha: {comentario.fecha_hora}")
    else:
